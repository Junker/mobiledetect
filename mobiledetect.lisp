...
(defvar *tablet-devices*
  '(;; @todo: check for mobile friendly emails topic.
    ...
    ;; http://docs.aws.amazon.com/silk/latest/developerguide/ua.html
...
(defvar *browsers*
  '(;;(:vivaldi . "Vivaldi")
    ;; @reference: https://developers.google.com/chrome/mobile/docs/ua
...
;; compile regexs
(flet ((compile-regex-alist (regex-list)
         (mapcar #'(lambda (pair)
                     (cons (car pair) (ppcre:create-scanner (cdr pair))))
                 regex-list)))
  (setf *phone-devices* (compile-regex-alist *phone-devices*))
  (setf *tablet-devices* (compile-regex-alist *tablet-devices*))
  (setf *browsers* (compile-regex-alist *browsers*))
  (setf *operating-system* (compile-regex-alist *operating-system*)))

;; ======== FUNCS

(defun match-regex-alist (var regex-list)
  (find var regex-list
        :test (lambda (v regex-pair)
                (ppcre:scan (cdr regex-pair) v))))


;; PUBLIC

(defun mobilep (ua)
  (when (match-regex-alist ua (append *operating-system*
                                      *browsers*
                                      *phone-devices*
                                      *tablet-devices*))
    t))
...
(defun tabletp (ua)
  (when (match-regex-alist ua *tablet-devices*)
    t))

(defun browser (ua)
  (car (match-regex-alist ua *browsers*)))

(defun operating-system (ua)
  (car (match-regex-alist ua *operating-system*)))

(defun mobile-device (ua)
  (car (match-regex-alist ua (append *phone-devices* *tablet-devices*))))

(defun tablet-device (ua)
  (car (match-regex-alist ua *tablet-devices*)))

(defun device-type (ua)
  (cond ((mobilep ua) :mobile)
        ((tabletp ua) :tablet)))
